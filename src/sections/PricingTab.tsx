      ...
      <div className="max-w-6xl mx-auto px-5 sm:px-8 lg:px-14 py-12">
        ...
        <div className="grid lg:grid-cols-5 gap-8 lg:gap-12">
          ...
          <div className="lg:col-span-3 space-y-6">
              ...
              <div className="bg-white rounded-2xl border border-neutral-200 p-6 sm:p-8">
                ...
                <div className="space-y-3">
                  {(reward.tier === '얼리버드' || reward.tier === '기본' ? [
                    ...
                    '디지털 선물에 실물 감성을 더하고 싶은 분',
                    '부모님께 USB를 직접 건네드리고 싶은 분',
            ...
            <FadeIn delay={0.12}>
              <div className="bg-white rounded-2xl border border-neutral-200 p-6 sm:p-8">
                <h3 className="text-lg font-bold text-ink mb-2">이런 노래가 만들어져요</h3>
                <p className="text-sm text-ink-muted mb-5">실제 제작된 샘플을 들어보세요</p>
                <div className="bg-gradient-to-br from-primary-50 to-primary-100/50 rounded-xl p-5 flex items-center gap-4">
                  <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-primary-400 to-primary-500 flex items-center justify-center flex-shrink-0 shadow">
                    <svg className="w-6 h-6 text-white ml-0.5" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-ink text-sm">식탁 위의 온기</p>
                    <p className="text-xs text-ink-muted mt-0.5">40대 아들 → 엄마 칠순 선물 · 오리지널</p>
                    <p className="text-xs text-ink-faint mt-1 italic">"칠순잔치에서 틀어드렸는데 온 가족이 울었어요."</p>
                  </div>
                </div>
              </div>
            </FadeIn>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {[
                        ...
                        ['수정 횟수', '별도 비용', '1회 무료'],
                        ['전달 방식', '파일 전송', '카카오톡 링크' + (needsAddress ? ' + USB' : '')],
                        ['작사/작곡', '별도 의뢰', '포함'],
            ...
            <FadeIn delay={0.16}>
              <div className="bg-white rounded-2xl border border-neutral-200 p-6 sm:p-8">
                ...
                <div className="space-y-5">
                  {[
                    { title: '식탁 위의 온기', from: '40대 아들 → 엄마 칠순', type: '오리지널', story: '매일 새벽 도시락 싸주시던 엄마에게 전하는 30년 치 감사를 담았습니다.', result: '칠순잔치에서 틀어드렸더니 온 가족이 울었다고 합니다.' },
                    { title: '바쁘다는 핑계', from: '30대 딸 → 아빠 팔순', type: '오리지널', story: '바쁘다는 핑계로 자주 못 찾아뵌 미안함과 고마움을 전했습니다.', result: '아버지가 차에서 매일 반복 재생하신다고 합니다.' },
                  ].map((c, i) => (
                    <div key={i} className="bg-neutral-50 rounded-xl p-5">
                      <div className="flex items-center justify-between mb-2">
                        <p className="font-bold text-ink text-sm">{c.title}</p>
                        <span className="text-xs text-primary-400 bg-primary-50 rounded-full px-2.5 py-0.5">{c.type}</span>
                      </div>
            ...
            <FadeIn delay={0.2}>
              <div className="bg-neutral-100 rounded-2xl p-6 sm:p-8">
                ...
                <ul className="space-y-1.5 text-xs text-ink-muted leading-relaxed">
                  ...
                  <li>· 커버곡의 경우 저작권 관련 사전 확인이 필요합니다</li>
                  <li>· 제작 시작 후에는 주문 취소가 제한됩니다</li>
                  <li>· USB 배송은 국내 배송만 가능합니다</li>
                </ul>
              </div>
            </FadeIn>
          </div>
...
function OrderForm({ reward, onBack }: { reward: typeof rewards[0]; onBack: () => void }) {
  const [form, setForm] = useState({
    ...
    songType: 'original' as 'original' | 'cover',
    coverSong: '', extra: '', address: '',
  })
        ...
        <div className="grid lg:grid-cols-5 gap-8 lg:gap-12">
          ...
          <div className="lg:col-span-3 space-y-6">
              ...
              <div className="bg-white rounded-2xl border border-neutral-200 p-6 sm:p-8">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" value={form.name} onChange={e => update('name', e.target.value)} placeholder="홍길동" className={inputClass} />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-ink-muted mb-1.5">연락처 <span className="text-primary-400">*</span></label>
                    <input type="tel" value={form.phone} onChange={e => { const raw = e.target.value.replace(/[^0-9]/g, '').slice(0, 11); const v = raw.length <= 3 ? raw : raw.length <= 7 ? `${raw.slice(0,3)}-${raw.slice(3)}` : `${raw.slice(0,3)}-${raw.slice(3,7)}-${raw.slice(7)}`; update('phone', v) }} placeholder="010-0000-0000" className={inputClass} />
                  </div>
                ...
                <div className="space-y-4">
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="text" value={form.recipient} onChange={e => update('recipient', e.target.value)} placeholder="엄마, 아빠, 배우자 등" className={inputClass} />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-ink-muted mb-1.5">받는 분과의 관계 <span className="text-primary-400">*</span></label>
                      <input type="text" value={form.relation} onChange={e => update('relation', e.target.value)} placeholder="아들, 딸, 남편, 아내 등" className={inputClass} />
                    </div>
                  </div>
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div>
                  ...
                  <div className="bg-neutral-50 rounded-xl p-5 space-y-4">
                    ...
                    <div>
                      ...
                      <textarea value={form.memory} onChange={e => update('memory', e.target.value)} rows={2} placeholder="예: 매일 새벽 도시락 싸주셨던 기억이 아직도 생생해요" className={`${inputClass} resize-none`} />
                    </div>
                    <div>
                      <p className="text-xs text-ink-muted mb-1">평소에 전하지 못한 말이 있다면?</p>
                      <textarea value={form.unsaid} onChange={e => update('unsaid', e.target.value)} rows={2} placeholder="예: 바쁘다는 핑계로 자주 못 찾아뵈서 항상 미안해요" className={`${inputClass} resize-none`} />
                    </div>
                    <div>
                      <p className="text-xs text-ink-muted mb-1">함께한 특별한 추억이 있나요?</p>
                      <textarea value={form.episode} onChange={e => update('episode', e.target.value)} rows={2} placeholder="예: 어릴 때 손잡고 학교 데려다주시던 길이 아직도 기억나요" className={`${inputClass} resize-none`} />
                    </div>
                    <div>
                      <p className="text-xs text-ink-muted mb-1">그 외 노래에 담고 싶은 이야기</p>
                      <textarea value={form.story} onChange={e => update('story', e.target.value)} rows={2} placeholder="자유롭게 적어주세요" className={`${inputClass} resize-none`} />
                    </div>
                  </div>
            ...
            {needsAddress && (
              <FadeIn delay={0.2}>
                <div className="bg-white rounded-2xl border border-neutral-200 p-6 sm:p-8">
                  <h3 className="text-base font-bold text-ink mb-5">배송 정보 (USB)</h3>
                  <textarea value={form.address} onChange={e => update('address', e.target.value)} rows={2} placeholder="서울특별시 중구 퇴계로36길 2, B245" className={`${inputClass} resize-none`} />
                </div>
              </FadeIn>
            )}
            ...
            {needsPhotos && (
              <FadeIn delay={0.25}>
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-5">
                  <p className="text-sm text-amber-800">뮤직비디오 제작을 위한 가족 사진은 결제 완료 후 이메일로 안내드립니다.</p>
                </div>
              </FadeIn>
            )}
          </div>
